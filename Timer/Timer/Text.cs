...
namespace Timer
{
    class Text
    {
        public static void DisplaySecondsAndMinutes(int Seconds, int Minutes)
        {
            if (Minutes > 1)
            {
                if (Seconds > 1) { Console.WriteLine("There are {0} minutes and {1} seconds left", Minutes, Seconds); }
                else { Console.WriteLine("There are {0} minutes and {1} second left", Minutes, Seconds); }
            }
            else if (Minutes == 1)
            {
                if (Seconds > 1) { Console.WriteLine("There is 1 minute and {0} seconds left", Seconds); }
                else { Console.WriteLine("There is 1 minute and {0} second left", Seconds); }
            }
            else
            {
                if (Seconds > 1) { Console.WriteLine("There is {0} seconds left", Seconds); }
